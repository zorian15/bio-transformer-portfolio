...
### The rung 2 to rung 3 contrast in full

Generated by `analyse_contrasts.py` into `results/contrasts.csv`; see
[Method](method.md) for why the test is Wilcoxon signed-rank rather than a paired
\(t\)-test, and what the assay-clustered interval is doing. `esm2_t12_35M_UR50D`:

| scheme | pairs | LoRA − rung 2 | win rate | Wilcoxon p | per-assay range | assay-clustered 95% CI |
|---|---:|---:|---:|---:|---:|---:|
| all | 108 | **+0.0287** | 62.0% | 0.0024 | +0.011 to +0.056 | [−0.032, +0.089] |
| `fold_random_5` | 36 | **+0.0557** | 77.8% | 0.00015 | +0.024 to +0.104 | [−0.051, +0.162] |
| `fold_contiguous_5` | 36 | +0.0148 | 55.6% | 0.30 | −0.010 to +0.042 | [−0.050, +0.079] |
| `fold_modulo_5` | 36 | +0.0154 | 52.8% | 0.50 | −0.026 to +0.075 | [−0.116, +0.147] |

Three columns, three readings, and they do not all say the same thing.

**Across these 108 configurations**, LoRA beats the trained head overall and
decisively on `fold_random_5`: 78% of those arms improve and \(p =
1.5 \times 10^{-4}\). On the two position-disjoint schemes the win rate is
53-56%, close enough to a coin flip that no test was going to rescue it.

**Across proteins, nothing excludes zero, including `fold_random_5`.** Every
clustered interval covers it. That is not a contradiction of the \(p\)-value, it
is a different question: the \(p\) asks whether LoRA won on these three assays,
the interval asks whether it would win on a fourth. With \(K = 3\) the answer
to the second question is that this cohort cannot say, and the per-assay range
column shows why: on `fold_modulo_5` the three assays run from −0.026 to +0.075,
which is a sign change, not noise around a mean.

So the honest summary of this rung has two halves. The **direction** of the
effect is confined to the split that leaks residue positions, which is the
finding. The **magnitude** is not established at the level a reader would want,
and a three-assay cohort is the reason. Widening the cohort, not running more
seeds, is what would change that: seeds are already averaged out, and a fourth
assay is worth more than a hundred more configurations on these three.
...
test folds per scheme, and the per-assay deltas change sign. The effective
sample size for any scheme-level statement is nearer 3 than 36, and the
assay-clustered intervals in the table above are the consequence: none of them
excludes zero.

An earlier version of this page reported clustered intervals here that did
exclude zero for `random`, taken from a percentile bootstrap over assays. **That
method is degenerate at three clusters** and those numbers should not be quoted;
[Method](method.md) explains why, and `results/contrasts.csv` carries the
corrected intervals. The qualitative caveat was right and the interval was not
conservative enough.
