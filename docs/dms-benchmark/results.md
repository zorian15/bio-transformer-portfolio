...
\(t\)-test, and what the clustered interval is doing. `esm2_t12_35M_UR50D`:

| scheme | pairs | LoRA − rung 2 | win rate | Wilcoxon p | assay-clustered 95% CI |
|---|---:|---:|---:|---:|---:|
| all | 108 | **+0.0287** | 62.0% | 0.0024 | [+0.011, +0.056] |
| `fold_random_5` | 36 | **+0.0557** | 77.8% | 0.00015 | [+0.024, +0.104] |
| `fold_contiguous_5` | 36 | +0.0148 | 55.6% | 0.30 | [−0.010, +0.041] |
| `fold_modulo_5` | 36 | +0.0154 | 52.8% | 0.50 | [−0.026, +0.075] |

The win rate is the column to read alongside the mean. On the two
position-disjoint schemes it sits at 53-56%, which is close to a coin flip, and
their clustered intervals straddle zero. Only `fold_random_5` has both a win rate
clearly above chance and an interval excluding it, and that is the scheme where
folds share almost every residue position.

Read together, the three columns say the same thing three ways, which is the
point of reporting all three: a mean can be carried by a handful of arms, a
\(p\)-value can be small because the pairs are treated as independent when they
are not, and a win rate near 50% is hard to dress up as an effect.
